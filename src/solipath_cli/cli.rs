...
impl SolipathCli {
    ...
    pub async fn run_solipath_command(&self, commands: &[String]) {
    ...
    async fn update_solipath(&self) {
        ...
        fs::rename(&original_executable, &renamed_executable).unwrap();
...
mod test {
    ...
    async fn run_solipath_update_and_check_download_size() {
        ...
        solipath_executable.push(format!("solipath{}", get_executable_file_extension(std::env::consts::OS)));
        ...
        solipath_cli.run_solipath_command(&["--update".to_string()]).await;
